function createConstraintLabId(tx) {
    return tx.run(
        'CREATE CONSTRAINT labIdUnique IF NOT EXISTS ON (l:Lab) ' +
...
function createConstraintResearcherId(tx) {
    return tx.run(
        'CREATE CONSTRAINT researcherIdUnique IF NOT EXISTS ON (r:Researcher) ' +
...
function createConstraintResearchAreaId(tx) {
    return tx.run(
        'CREATE CONSTRAINT researchAreaIdUnique IF NOT EXISTS ON (ra:ResearchArea) ' +
...
function createConstraintProductId(tx) {
    return tx.run(
        'CREATE CONSTRAINT productIdUnique IF NOT EXISTS ON (p:Product) ' +
...
function createConstraintFacultyId(tx) {
    return tx.run(
        'CREATE CONSTRAINT facultyIdUnique IF NOT EXISTS ON (f:Faculty) ' +
...
function createConstraintArticleId(tx) {
    return tx.run(
        'CREATE CONSTRAINT articleIdUnique IF NOT EXISTS ON (p:Article) ' +
...
function createConstraintResearchId(tx) {
    return tx.run(
        'CREATE CONSTRAINT researchIdUnique IF NOT EXISTS ON (r:Research) ' +
...
function createConstraintResearchSetupId(tx) {
    return tx.run(
        'CREATE CONSTRAINT researchSetupIdUnique IF NOT EXISTS ON (rs:ResearchSetup) ' +
...
function createGraphConstraints(session) {
    return session
        .writeTransaction((tx) => createConstraintLabId(tx))
